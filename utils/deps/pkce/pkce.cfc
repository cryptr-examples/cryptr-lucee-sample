...
component accessors=true {
	...
	public string function generatePKCECodeVerifier(){
		...
		// var base64Encoder = createObject("java", "java.util.Base64").getUrlEncoder();
		...
		return base64Encoder.encodeToString(codeVerifier).replace("=", "", "all");
	...
	public string function generatePKCECodeChallenge( required string codeVerifier ){
		...
		return base64Encoder.encodeToString(messageDigest.digest()).replace("=", "", "all");
